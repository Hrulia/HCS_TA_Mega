...
float mTminSysPodacha; // = 33; //минимальная температура подачи системы (для зимы 33) 
//(возможно стоит ее изменять в зависимости от температуры на улице. если выше 0 то столько - 30. Если выше 5 = 27, если выше 10 = 23)
...
int temperatureControlSYS() {
	...
	if (PID_SYS == 1) {
		...
		// 13-t14, температура на улице
		if (temperature[13] < 0) { mTminSysPodacha = 33; } //вариант для зимы)	
		if (temperature[13] < 10) { mTminSysPodacha = 34; } //вариант для зимы)	//35 в большой спальне было жарко ночью
