...
const int tneedTTK = 60;	//требуемая температура обратки ТТК(60 * С)
...
int temperatureControlTTK() {
	...
	//читаем температуру терпмопары (температура дымовых газов ТТК)
	...
	///if (g_failure || (g_t_flueGas > 120) || ((int)temperature[3] > ((int)temperature[11] + 2))) { //авария, или котел топиться, или вода в подаче котла нагрелась более чем на 2 *С выше температуры воды низа ТА
...
int temperatureControlSYS() {
	...
	if (PID_SYS == 1) {
		...
		if ((temperature[12] < (tRoomSetpoint - 0.3)) && (digitalRead(PIN_VALVE_SYS_SIGNAL_ClOSE))) {
			...
			if ((temperature[12] > (tRoomSetpoint + 0.3)) && (digitalRead(PIN_VALVE_SYS_SIGNAL_OPEN))) {
