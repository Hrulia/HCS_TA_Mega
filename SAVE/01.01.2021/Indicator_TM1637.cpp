...
int indicator_TM1637_Output_temperature()
{
	...
	extern int g_t_flueGas;
	TM1637_7.showNumberMsn(g_t_flueGas,0);//временно выведем дымовые газы на чужой индикатор

	TM1637_8.showNumberMsn(temperature[15]);
	TM1637_9.showNumberMsn(g_tRoomSetpoint,0);
	...
	TM1637_11.showNumberMsn(temperature[13]);
		...
		while (buttonReinitTemp.flagPress) {
			...
			digitalWrite(13, HIGH);
				g_tRoomSetpoint++;
				if (g_tRoomSetpoint > 26) { g_tRoomSetpoint = 17;}
				TM1637_9.showNumberMsn(g_tRoomSetpoint, 0);
			delay(800);
			buttonReinitTemp.scanState();
