...
int temperatureControlTTK() {
	...
	if (g_failure || (g_t_flueGases > 120) || ((int)temperature[3] > ((int)temperature[11] + 3))) { //авария, или котел топиться, или вода в подаче котла нагрелась более чем на 5 *С выше температуры воды низа ТА
...
int temperatureControlSYS() {
	...
	if (PID_SYS == 1) {
		...
		//t[12] - температура в помещении.	
		//new:дополнительно контролируем, что бы температура в системе не опускалась ниже 33 градусов, что бы в доме не появлялся холод от окон
