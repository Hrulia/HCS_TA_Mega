...
const int tneed = 30;	//требуемая температура обратки ТТК(50 * С)

int t_flueGas = 200; //температура дымовых газов ТТК котла ()  

////uint8_t g_tRoomSetpoint = 20; //заданное значение температуры помещения (к которому стремимся)
...
int temperatureControlTTK() {
	...
	if (g_failure || ((int)temperature[3] > ((int)temperature[11] + 10))) { //котел топиться или вода в подаче котла нагрелась более чем на 10 *С выше температуры воды низа ТА
...
int temperatureControlSYS() {
	...
	if (g_failure ||((int)temperature[10] > ((int)temperature[12] + 10))) { //Температура верха ТА выше температуры в отслеживаемом помещении
		//запуск насоса системы
		LOG("zapusk nasosa SYS");
		digitalWrite(PIN_PUMP_SYS, LOW); //active level - LOW
	}
	else {//остановка насоса системы
